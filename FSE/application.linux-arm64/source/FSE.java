...
public class FSE extends PApplet {
...
float speedUp=0.1f;//increasing speed
...
public void scaling(){ //find scale factors
  scaleFactor[2] = PApplet.parseFloat(width)/1280.0f; //scaling on x axis
  scaleFactor[3] = PApplet.parseFloat(height)/720.0f; //scaling on y axis
...
public void movePlayer() {
  ...
  if (groundPos[nextGround][0]-PApplet.parseInt((PApplet.parseInt(speed)/3+5)*scaleFactor[1])*2 <= pos[0]+character[0].width/2 && bottomOfPlayer-topOfNextGrass >= 10*scaleFactor[1] && groundPos[nextGround][0] > 0){ //checks if the player is going to collide with the ground
    pos[0]-= PApplet.parseInt((PApplet.parseInt(speed)/3+6)*scaleFactor[1])*2; //pushes the player back
    colliding = true; //sets collision to true
  }
  else{
    colliding = false; //sets collision to false
  }
  if (pos[0] < PApplet.parseInt(width*(500.0f/1280)) && pos[0]>0){ //finds if the player was pushed back
  ...
  for (int i = 0; i < obstacles.size(); i++){ //goes through every obstacle to detect collisions
    float top = obstacles.get(i).get(1) - obstacleImages[PApplet.parseInt(obstacles.get(i).get(2))].height*((obstacles.get(i).get(2)+1)/2);//
    float left = obstacles.get(i).get(0)-obstacleImages[PApplet.parseInt(obstacles.get(i).get(2))].width/3;//left of obstacle
    float right = obstacles.get(i).get(0)+obstacleImages[PApplet.parseInt(obstacles.get(i).get(2))].width/3;//right of obstacle
    float futureLeft = left-PApplet.parseInt((PApplet.parseInt(speed)/3+5)*scaleFactor[1])*2;
    ...
    if (left > pos[0]+character[0].width/2 && futureLeft <= pos[0]+character[0].width/2 && right > pos[0]-character[0].width/2 && (top < bottomOfPlayer || pos[1] > top || pos[1]-character[0].height/2 > top)){
      pos[0] -= PApplet.parseInt((PApplet.parseInt(speed)/3+6)*scaleFactor[1])*3;
    }
  ...
  rightOfObstacle -= PApplet.parseInt((PApplet.parseInt(speed)/3+5)*scaleFactor[1])*2;//finds the new position of right of obstacle
...
public void initFont() {//initializing all fonts
...
public void initImgs() {//adding all images and resizing to proper size and scaled to the screen size
  armour = loadImage("Imgs/Armour.png");
  armour.resize(PApplet.parseInt(armour.width*3/4*scaleFactor[0]), PApplet.parseInt(armour.height*3/4*scaleFactor[0]));
  ...
  for (int i = 0; i < guns.length; i++){
    for (int j = 0; j < guns[i].length; j++){
      guns[i][j].resize(PApplet.parseInt(guns[i][j].width*scaleFactor[0]), PApplet.parseInt(guns[i][j].height*scaleFactor[0]));
    }
  }
  ...
  for(int i = 0; i < background.length; i++){
    background[i].resize(PApplet.parseInt(background[i].width*scaleFactor[1]), PApplet.parseInt(background[i].height*scaleFactor[1]));
  }
  
  bullet = loadImage("Imgs/Bullet.png");
  bullet.resize(PApplet.parseInt(bullet.width*5*scaleFactor[0]), PApplet.parseInt(bullet.height*5*scaleFactor[0]));
  
  rocket = loadImage("Imgs/RPG Ammo.png");
  rocket.resize(PApplet.parseInt(rocket.width*scaleFactor[0]), PApplet.parseInt(rocket.height*scaleFactor[0]));
  
  obstacleImages[0] = loadImage("Imgs/Tree.png");
  obstacleImages[1] = loadImage("Imgs/Bricks.png");
  
  obstacleImages[0].resize(PApplet.parseInt(obstacleImages[0].width*scaleFactor[0]), PApplet.parseInt(obstacleImages[0].height*scaleFactor[0]));
  obstacleImages[1].resize(PApplet.parseInt(obstacleImages[1].width*scaleFactor[0]), PApplet.parseInt(obstacleImages[1].height*scaleFactor[0]));
  ...
  for (int i = 0; i < 5; i++) {
    character[i].resize(PApplet.parseInt(character[i].width*3/4*scaleFactor[0]), PApplet.parseInt(character[i].height*3/4*scaleFactor[0]));
    if(i<3){
      robot[i].resize(PApplet.parseInt(robot[i].width*3/4*scaleFactor[0]), PApplet.parseInt(robot[i].height*3/4*scaleFactor[0]));
    }
  }
  ...
  mainMenuPic.resize(PApplet.parseInt(PApplet.parseFloat(mainMenuPic.width)*scaleFactor[0]*(1280.0f/1920)), PApplet.parseInt(PApplet.parseFloat(mainMenuPic.height)*scaleFactor[0]*(1280.0f/1920)));
  coin = loadImage("Imgs/Coin.png");
  coin.resize(PApplet.parseInt(coin.width*scaleFactor[0]), PApplet.parseInt(coin.height*scaleFactor[0]));
}
  ...
  for (int i = 0; i < shopOptions.length; i++){
    ...
    if (i < 2){ //checks if i is less than 2
      ...
      for (int j = 0; j < buffer.length; j++){
        intBuffer[j]=PApplet.parseInt(buffer[j]); //converts string to int
      }
      shopCosts[i] = intBuffer; //stores store prices
    }
  }
  initImgs(); //intializes all the pictures
  pos[0]=PApplet.parseInt(width*(500.0f/1280)); //sets the position of player according the scaling of screen
  groundPos[0][1] = PApplet.parseInt(500.0f*scaleFactor[0]); //sets vertical ground position of one grounds
  groundPos[1][0] = PApplet.parseInt(background[0].width); //sets the 2nd ground position to the length of the ground picture
  groundPos[1][1] = random(-70*scaleFactor[0], 70*scaleFactor[0])+500.0f*scaleFactor[0]; //sets the vertical height of the 2nd ground position
  skyX[1] = width;
  speedBoost = gravity*PApplet.parseFloat(PApplet.parseInt(saveData[7])/2); //finds the speedboost for jetpack
  maxFuel = 100+((PApplet.parseInt(saveData[7])+1)/2)*20;
  maxHealth = 100 + PApplet.parseInt(saveData[2])*20;
...
public void mainMenu() {//initializing main menu
  ...
  for (int i = 0; i < 4; i++){
    ...
    if (buttons[i]){ //checks if mouse is over button
      ...
      if(clicked){ //checks if button is clicked
        ...
        if(i == 0) bulletsRemaining = bullets[PApplet.parseInt(saveData[1])]; //sets bullets remaining to the current weapon
      }
    }
  ...
  if (PApplet.parseInt(saveData[0]) == 0){ //checks if the distance travelled is 0
...
public void mousePressed(){//checks if mouse was pressed
  clicked = true;
}

public void mouseReleased(){//checks if mouse was released
  clicked = false;
}

public void keyReleased() {//checks if key was released
  if (key == 'w'){
    justJumped = false;
    holding = false;
  }
...
public void addTrail() {//adding trail
...
public void jetpack() {//adding jetpack
  ...
  if (keyPressed && key==32 && fuel >= 0) {//checks if space key is pressed
    ...
    if (pos[1] > -character[0].height/2){ //checks if weapon is lower than the top of the screen
      vy=(-3.0f*gravity-speedBoost)*scaleFactor[0];//player goes up
      onGround = false; //sets onGround to false
...
public void updateTrail() { //updates trail and draws trail
  for (int i = 0; i < trail.size(); i++) { //goes through the size of all the trails in trails list
    ...
    if (PApplet.parseInt(saveData[1]) <= 1){
      image(guns[PApplet.parseInt(saveData[1])][0], guns[PApplet.parseInt(saveData[1])][0].width/5, character[1].height);
      image(guns[PApplet.parseInt(saveData[1])][1], guns[PApplet.parseInt(saveData[1])][0].width/2+guns[PApplet.parseInt(saveData[1])][1].width/4, character[1].height-trail.get(i).get(3));
    }
    else if (PApplet.parseInt(saveData[1]) <= 3 || PApplet.parseInt(saveData[1]) == 8) image(guns[PApplet.parseInt(saveData[1])][0], 0, character[1].height);
    else if (PApplet.parseInt(saveData[1]) == 4) image(guns[4][0], 0, character[1].height+guns[4][0].height/6);
    else if (PApplet.parseInt(saveData[1]) == 5) image(guns[5][0], guns[5][0].width/5, guns[5][0].height/3 + character[1].height);
    else if (PApplet.parseInt(saveData[1]) == 6) image(guns[6][0], 0, character[1].height+guns[6][0].height/4);
    else if (PApplet.parseInt(saveData[1]) == 7) image(guns[7][0], guns[7][0].width/4, character[1].height-guns[7][0].width/6);
    else if (PApplet.parseInt(saveData[1]) <=14) image(guns[PApplet.parseInt(saveData[1])][0], 0, character[1].height-guns[PApplet.parseInt(saveData[1])][0].height/10);
    else{
      switch(PApplet.parseInt(saveData[1])){
        case 15:
    ...
    trail.get(i).set(0, trail.get(i).get(0)+PApplet.parseInt((PApplet.parseInt(speed)/3-10)*scaleFactor[1])*2); //moves trails back a bit
...
public void addBullet(){ //function to add bullets
  ...
  if (PApplet.parseInt(saveData[1]) < 16) newBullet.add(pos[1]-character[1].width/2);
  else if (PApplet.parseInt(saveData[1]) <= 17) newBullet.add(pos[1]-character[1].width/2-guns[PApplet.parseInt(saveData[1])][0].width/5);
  else newBullet.add(pos[1] - character[1].width/2);
  newBullet.add(0.0f); //adds variable to determine how many objects sniper rifle has gone through
  bulletPos.add(newBullet); //adds newBullet to bulletPos
}

public void drawBullet(){//draws bullets
  ...
  for(int i = 0; i < bulletPos.size(); i++){ //loop goes through every bullet in bulletPos
    if (PApplet.parseInt(saveData[1]) < 18) image(bullet, bulletPos.get(i).get(0), bulletPos.get(i).get(1)); //checks if gun isn't an rpg, and shows a bullet
  ...
  for (int i = 0; i < enemyBullets.size(); i++){ //llops goes through every bullet in enemyBullets
    if (PApplet.parseInt(saveData[1]) < 18) image(bullet, enemyBullets.get(i).get(0), enemyBullets.get(i).get(1)); //checks if gun isn't an rpg, and shows a bullet
    else image(rocket, enemyBullets.get(i).get(0), enemyBullets.get(i).get(1)); //else shows a rocket
    enemyBullets.get(i).set(0, enemyBullets.get(i).get(0)-PApplet.parseInt((PApplet.parseInt(speed)/3+40)*scaleFactor[1])*2); //moves bullet
    if (enemyBullets.get(i).get(0)-bullet.width/2 < 0){ //check if bullet goes beyond screen
      enemyBullets.remove(i); //removes bullet
    }
    else if (enemyBullets.get(i).get(0) > pos[0]+character[0].width/2 && enemyBullets.get(i).get(0)-PApplet.parseInt((PApplet.parseInt(speed)/3+40)*scaleFactor[1])*2 <= pos[0]+character[0].width/2 && enemyBullets.get(i).get(1) > pos[1]-character[0].height/2 && enemyBullets.get(i).get(1) < pos[1]+character[0].height/3+character[2].height){ //checks if bullet will hit player
      enemyBullets.remove(i); //removes bullet
      health -= dmg[PApplet.parseInt(saveData[1])]; //decrease health of player
    }
  }
}
...
public void drawArms() {//adding arms on player
  ...
  if (mousePressed && !justFired && bulletsRemaining > 0 && lastShot + fireRate[PApplet.parseInt(saveData[1])] <= millis()) {//checks if able to to fire and player clicked
    vRecoil = 2*scaleFactor[0];//sets recoil
    recoil=2*scaleFactor[0];
    lastShot = millis();//last shot current time
    if (!auto[PApplet.parseInt(saveData[1])]) justFired = true;//checks if semi-automatic weapon
  ...
  } else if (!mousePressed && !auto[PApplet.parseInt(saveData[1])]) {//checks if mouse is released and is semi auto
  ...
  if ((recoil != 0) || (PApplet.parseInt(saveData[1]) >= 2 && ((mousePressed && auto[PApplet.parseInt(saveData[1])])) || (auto[PApplet.parseInt(saveData[1])] == false && justFired)) && pos[0] > 0 && !reloading) { //checks if gun was just shot or recoil is not 0 and character is on screen
  ...
  if (PApplet.parseInt(saveData[1]) <= 1){
    image(guns[PApplet.parseInt(saveData[1])][0], guns[PApplet.parseInt(saveData[1])][0].width/5, character[1].height);
    image(guns[PApplet.parseInt(saveData[1])][1], guns[PApplet.parseInt(saveData[1])][0].width/2+guns[PApplet.parseInt(saveData[1])][1].width/4, character[1].height-recoil);
  }
  else if (PApplet.parseInt(saveData[1]) <= 3 || PApplet.parseInt(saveData[1]) == 8) image(guns[PApplet.parseInt(saveData[1])][0], 0, character[1].height);
  else if (PApplet.parseInt(saveData[1]) == 4) image(guns[4][0], 0, character[1].height+guns[4][0].height/6);
  else if (PApplet.parseInt(saveData[1]) == 5) image(guns[5][0], guns[5][0].width/5, guns[5][0].height/3 + character[1].height);
  ...
  if (bulletsRemaining == 0 && millis()-reloadStart >= reloadTime[PApplet.parseInt(saveData[1])]) { //checks if bulletsRemaining is none and checks if finished reloading
    bulletsRemaining = bullets[PApplet.parseInt(saveData[1])]; //bulletsRemaing
...
public void charInfo() {
  ...
  text(PApplet.parseInt(distTravelled)+" m", 100*scaleFactor[0], 100*scaleFactor[0]);//shows distance travelled
  textAlign(RIGHT);//changing text align
  text("Money: $" + String.format("%,d", PApplet.parseInt(saveData[3])), width-48*scaleFactor[0], 48*scaleFactor[0]);//shows money the player has
  rectMode(CENTER);//changing rect mode
  fill(0);//adding fill
...
public void drawChar() {//draws character
  if (trail.size() == 0 || trail.get(trail.size()-1).get(0) <= pos[0]-character[0].width*1.75f || trail.get(trail.size()-1).get(1) > pos[1]+character[0].height*1.75f || trail.get(trail.size()-1).get(1)+character[0].height*1.75f < pos[1]) addTrail();//checks if able to add trail
  imageMode(CENTER);//changing image mode
  ...
  if (PApplet.parseInt(saveData[2]) > 0) image(armour, pos[0], pos[1]+character[0].height/2-armour.height/2); //if armour is bought, it shows armour
...
public void updateEnemies(){ //update and draws Enemies
  ...
  for (int i = 0; i < enemies.size(); i++){ //loops through all list of enemies
    ...
    //draws gun
    if (PApplet.parseInt(saveData[1]) <= 1){
      image(guns[PApplet.parseInt(saveData[1])][0], guns[PApplet.parseInt(saveData[1])][0].width/5, 0);
      image(guns[PApplet.parseInt(saveData[1])][1], guns[PApplet.parseInt(saveData[1])][0].width/2+guns[PApplet.parseInt(saveData[1])][1].width/4, -enemies.get(i).get(4));
    }
    else if (PApplet.parseInt(saveData[1]) <= 3 || PApplet.parseInt(saveData[1]) == 8) image(guns[PApplet.parseInt(saveData[1])][0], 0, 0);
    else if (PApplet.parseInt(saveData[1]) == 4) image(guns[4][0], 0, guns[4][0].height/6);
    else if (PApplet.parseInt(saveData[1]) == 5) image(guns[5][0], guns[5][0].width/5, guns[5][0].height/3);
    else if (PApplet.parseInt(saveData[1]) == 6) image(guns[6][0], 0, guns[6][0].height/4);
    else if (PApplet.parseInt(saveData[1]) == 7) image(guns[7][0], guns[7][0].width/4, guns[7][0].width/6);
    else if (PApplet.parseInt(saveData[1]) <=14) image(guns[PApplet.parseInt(saveData[1])][0], 0, guns[PApplet.parseInt(saveData[1])][0].height/10);
    else{
      switch(PApplet.parseInt(saveData[1])){
        case 15:
    ...
    if (millis()-enemies.get(i).get(2) >= robotReload[PApplet.parseInt(saveData[1])] && enemies.get(i).get(0) < width){ //checks if gun is able to shoot and enemy is on screen
      ...
      if (PApplet.parseInt(saveData[1]) < 16)newBullet.add(enemies.get(i).get(1)-robot[1].width/6);
      else if (PApplet.parseInt(saveData[1]) <= 17)newBullet.add(enemies.get(i).get(1)-robot[1].width/6-guns[PApplet.parseInt(saveData[1])][0].width/5);
      else newBullet.add(enemies.get(i).get(1)-robot[1].width/6);
      
      enemyBullets.add(newBullet); //adds newBullet to enemyBullets
      enemies.get(i).set(4, 2.0f); //add recoil and gun shot
      enemies.get(i).set(2, PApplet.parseFloat(millis())); //record time last shot
    }
    ...
    if (!(enemies.get(i).get(0)-PApplet.parseInt((PApplet.parseInt(speed)/3+10)*scaleFactor[1])*2>groundPos[nextGroundEnemy][0] && enemies.get(i).get(0)-PApplet.parseInt((PApplet.parseInt(speed)/3+10)*scaleFactor[1])*2 < groundPos[nextGroundEnemy][0]+background[0].width && bottomEnemy-nextTopGroundEnemy>5*scaleFactor[3])) enemies.get(i).set(0, enemies.get(i).get(0)-PApplet.parseInt((PApplet.parseInt(speed)/3+10)*scaleFactor[1])*2); //checks if robot will collide with ground and pushes robot back
    rectMode(CENTER);
...
public void drawUpdateObstacle(){ //drawing and updating obstacles
  imageMode(CENTER);
  for(int i = 0; i < obstacles.size(); i++){
    image(obstacleImages[PApplet.parseInt(obstacles.get(i).get(2))], obstacles.get(i).get(0), obstacles.get(i).get(1)-obstacleImages[PApplet.parseInt(obstacles.get(i).get(2))].height/2); //drawing obstacle according to what obstacle it is
    obstacles.get(i).set(0, obstacles.get(i).get(0)-PApplet.parseInt((PApplet.parseInt(speed)/3+5)*scaleFactor[1])*2); //moves obstacle towards player
    if (obstacles.get(i).get(0)+obstacleImages[PApplet.parseInt(obstacles.get(i).get(2))].height/2 <= 0){ //checks if obstacle is beyond screen and removes it
      obstacles.remove(i);
      i--;
    }
  }
...
public void detectCollision(){ //detecting collisions
  for(int i = 0; i < bulletPos.size(); i++){//loops going through bullets
    ...
    for (int j = 0; j < obstacles.size(); j++){ //loops through all obstacles
      ...
      float top = obstacles.get(j).get(1) - obstacleImages[PApplet.parseInt(obstacles.get(j).get(2))].height;
      float left = obstacles.get(j).get(0) - obstacleImages[PApplet.parseInt(obstacles.get(j).get(2))].width/2;
      float right = obstacles.get(j).get(0) + obstacleImages[PApplet.parseInt(obstacles.get(j).get(2))].width/2;
    ...
    if (bulletPos.get(i).get(0)+60*scaleFactor[0] >= closest && bulletPos.get(i).get(1) > topOfClosest && bulletPos.get(i).get(1) < bottomOfClosest){
      if (value[1] == 1){ //checks if colliding with an enemy
        enemies.get(value[0]).set(3, enemies.get(value[0]).get(3)-dmg[PApplet.parseInt(saveData[1])]); //subtracts damage from enemy health
        if (enemies.get(value[0]).get(3) <= 0){ //checks if enemy health is below zero, and remove enemy and reset all values
          ...
          closest = 100000;
          rightClosest = 100000;
          saveData[3] = Integer.toString(PApplet.parseInt(saveData[3])+PApplet.parseInt(random(0, 11))*100);
        }
      }
      else{ //since it's not enemy, it must be colliding with an obstacle
        obstacles.get(value[0]).set(3, obstacles.get(value[0]).get(3)-dmg[PApplet.parseInt(saveData[1])]); //removes damage from obstacle health
        obstacles.get(value[0]).set(1, obstacles.get(value[0]).get(1)+obstacleImages[PApplet.parseInt(obstacles.get(value[0]).get(2))].height*(dmg[PApplet.parseInt(saveData[1])])/100); //lowers obstacle a bit
      ...
      if (PApplet.parseInt(saveData[1]) >= 18){ //checks if weapon is rpg
      ...
      if ((PApplet.parseInt(saveData[1]) != 15 && PApplet.parseInt(saveData[1]) != 14) || (bulletPos.get(i).get(2) >= 3)) bulletPos.remove(i); //checks if weapon isn't sniper, or has gone through 3 objects so it can remove bullet
  ...
  for (int i = 0; i < enemies.size(); i++){ //goes through all enemy values
    ...
    float futureLeft = enemies.get(i).get(0)-robot[0].width/2-PApplet.parseInt((PApplet.parseInt(speed)/3+10)*scaleFactor[1])*2;
...
public void updateCoins(){ //function to update and draw coins
  ...
  for (int i = 0; i < coins.size(); i++){ //loop through all the coins
    ...
    coins.get(i).set(0, coins.get(i).get(0)-PApplet.parseInt((PApplet.parseInt(speed)/3+5)*scaleFactor[1])*2); //moves coins back a bit
    ...
    if (coins.get(i).get(1)-25*scaleFactor[0]>pos[1]-character[0].height/2&& coins.get(i).get(0)<pos[0]+character[0].width/2 && coins.get(i).get(0) > pos[0]-character[0].width/2 && coins.get(i).get(1)-25 < pos[1]+character[0].height/3+character[2].height){
      coins.remove(i);
      i--;
      saveData[3] = Integer.toString(PApplet.parseInt(saveData[3])+PApplet.parseInt(random(5,11))*100);
    }
...
public void game() { //function for actual game
  imageMode(CENTER);
  ...
  for (int i = 0; i < skyX.length; i++){ //going through all sky positions
    ...
    if (!firstTime) skyX[i]-=PApplet.parseInt((PApplet.parseInt(speed)/4+1)*scaleFactor[1])+1; //moves skyX back only if not firstTime
  ...
  for (int i = 0; i < groundPos.length; i++) { //loops through the size of groundPos
    ...
    if (groundPos[i][0] <= -background[0].width) { //checks if the ground is going beyond the screen
      ...
      int randomNumber = PApplet.parseInt(random(0, 4));
      for (int a = 0; a < randomNumber; a++){
        ArrayList<Float> newObstacle = new ArrayList<Float>();
        float whatObstacle = PApplet.parseInt(random(0, 2));
      ...
      for (int a = 0; a < numOfEnemies; a++){
        ...
        newEnemy.add(PApplet.parseFloat(millis()));
        newEnemy.add(100.0f);
        newEnemy.add(0.0f);
        newEnemy.add(2.0f);
      ...
      int numOfCoins = PApplet.parseInt(random(0, 4));
    ...
    if (!firstTime) groundPos[i][0]-=PApplet.parseInt((PApplet.parseInt(speed)/3+5)*scaleFactor[1])*2; //updates ground position if not first time, and moves ground back
  }
  ...
  if (!firstTime){ // checks if it not first time and updates player info, and other controls
    movePlayer();
    distTravelled=distTravelled+0.04f*speed;
    ...
    text("Rules to the game", width/2, height/2-height*0.4f+100*scaleFactor[0]);
  ...
  if(pos[0]<=0 || health <= 0){ //shows player stats if dead or beyond screen
    ...
    text("Distance: "+PApplet.parseInt(distTravelled)+"m",width/2,height/2);
    int highscore = PApplet.parseInt(saveData[0]);
    if (PApplet.parseInt(distTravelled) > highscore) highscore = PApplet.parseInt(distTravelled);
...
public void credits() { //loads credits, and license to font
  background(100);
  imageMode(CENTER);
...
int[] selection = {PApplet.parseInt(saveData[1]), PApplet.parseInt(saveData[2]), PApplet.parseInt(saveData[7])}; //values of store selection

public void shop(){ //game shop
  ...
  text("Money: $" + String.format("%,d", PApplet.parseInt(saveData[3])), width-70*scaleFactor[0], 70*scaleFactor[0]);
  ...
  for (int i = 0; i < shopOptions[0].length; i++){ //goes through all shopOptions and displays them
    ...
    if (i!=2){
      String price;
      if (selection[i] > PApplet.parseInt(saveData[i+4])+1) price = "LOCKED"; //if previous item hasn't been unlocked
      else if (selection[i] > PApplet.parseInt(saveData[i+4]))price = "$" + String.format("%,d", shopCosts[i][selection[i]]); //shows price of weapon if item is avaliable to be unlocked
      else if (selection[i] == PApplet.parseInt(saveData[i+1]))price = "SELECTED"; //shows item that's already being selected
      ...
      if (mouseX >= width/2-40*scaleFactor[0] && mouseX <= width/2+40*scaleFactor[0] && mouseY >= i*170*scaleFactor[0]+210*scaleFactor[0] && mouseY <= i*170*scaleFactor[0]+290*scaleFactor[0]){
        ...
        if (clicked){ //shows if it's being clicked
          if (selection[i] <= PApplet.parseInt(saveData[i+4]) || (PApplet.parseInt(saveData[3]) >= shopCosts[i][selection[i]] && selection[i] == PApplet.parseInt(saveData[i+4])+1)){ //switches item if it's avaliable 
            saveData[i+1] = Integer.toString(selection[i]);
            maxHealth = 100 + PApplet.parseInt(saveData[2])*20;
            health = maxHealth;
          }
          if (PApplet.parseInt(saveData[3]) >= shopCosts[i][selection[i]] && selection[i] == PApplet.parseInt(saveData[i+4])+1){ //removes money if item is purchasable
            saveData[3] = Integer.toString(PApplet.parseInt(saveData[3]) - shopCosts[i][selection[i]]);
            saveData[i+4] = Integer.toString(PApplet.parseInt(saveData[i+4])+1);
          }
      ...
      if (i == 0){ //shows all gun pictures in shop
          ...
          switch(selection[0]){
            ...
            case 1:
              ...
              break;
            default:
              scale(75.0f*scaleFactor[0]/guns[selection[0]][0].height);
      ...
      if (areas1[0]  + areas1[1] + areas1[2]== abs(((width/2+50*scaleFactor[0])-(width/2+50*scaleFactor[0]))*((i*170*scaleFactor[0]+250*scaleFactor[0])-(i*170*scaleFactor[0]+210*scaleFactor[0])) - ((width/2+90*scaleFactor[0])-(width/2+50*scaleFactor[0]))*((i*170*scaleFactor[0]+290*scaleFactor[0]) - (i*170*scaleFactor[0]+210*scaleFactor[0])))){
        fill(127);
        if (clicked && (selection[i] < shopOptions[i+1].length-1)||(i == 2 && selection[i]<PApplet.parseInt(saveData[6]))) selection[i]++;
      }
      else fill (255);
      beginShape();
      vertex(width/2+50*scaleFactor[0], i*170*scaleFactor[0]+210*scaleFactor[0]);
      ...
      if (mouseX >= width/2-150*scaleFactor[0] && mouseX <= width/2+150*scaleFactor[0] && mouseY >= i*170*scaleFactor[0]+210*scaleFactor[0] && mouseY <= i*170*scaleFactor[0]+290*scaleFactor[0]){
        ...
        if (clicked){ //checks if button is pressed
          if (selection[i] <= PApplet.parseInt(saveData[i+4]) || PApplet.parseInt(saveData[3]) >= selection[2]*5000){ //if item is avaliable, it switches item
            saveData[7] = Integer.toString(selection[i]);
            maxFuel = 100+((PApplet.parseInt(saveData[7])+1)/2)*20;
            speedBoost = gravity*(PApplet.parseInt(saveData[7])/2.0f);
            fuel = maxFuel;
          }
          if (PApplet.parseInt(saveData[3]) >= selection[2]*5000 && selection[i] > PApplet.parseInt(saveData[i+4])){ //if item can be purchased, money is removed
            saveData[3] = Integer.toString(PApplet.parseInt(saveData[3]) - 5000*selection[2]);
            saveData[i+4] = Integer.toString(PApplet.parseInt(saveData[i+4])+1);
          }
      ...
      if (selection[2] == PApplet.parseInt(saveData[7]) )text("(SELECTED)", width/2, i*170*scaleFactor[0]+275*scaleFactor[0]);
      else if (selection[2] > PApplet.parseInt(saveData[i+4])) text("($" + String.format("%,d", selection[2]*5000) + ")", width/2, i*170*scaleFactor[0]+275*scaleFactor[0]);
      ...
      if (areas1[0]  + areas1[1] + areas1[2]== abs(((width/2+160*scaleFactor[0])-(width/2+160*scaleFactor[0]))*((i*170*scaleFactor[0]+250*scaleFactor[0])-(i*170*scaleFactor[0]+210*scaleFactor[0])) - ((width/2+200*scaleFactor[0])-(width/2+160*scaleFactor[0]))*((i*170*scaleFactor[0]+290*scaleFactor[0]) - (i*170*scaleFactor[0]+210*scaleFactor[0])))){
        fill(127);
        if (clicked && selection[i] <= PApplet.parseInt(saveData[i+4]))selection[i]++;
      }
      else fill (255);
      beginShape();
      vertex(width/2+160*scaleFactor[0], i*170*scaleFactor[0]+210*scaleFactor[0]);
...
public void draw() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "FSE" };
    if (passedArgs != null) {
